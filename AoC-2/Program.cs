...
namespace Two_Passwords
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                var components = policy.Split(' ');
                var boundAmounts = components[0].Split('-');
                var lowerBound = Convert.ToInt32(boundAmounts[0]);
                var upperBound = Convert.ToInt32(boundAmounts[1]);
                var character = Convert.ToChar(components[1].Substring(0, 1));
                var password = components[2];

                // Part 1
                
                var numOfCharsInPassword = password.Split(character).Length - 1;
                if (numOfCharsInPassword >= lowerBound && numOfCharsInPassword <= upperBound)
                {
                    validPasswords1++;
                }

                // Part 2
                var lowerBoundMatch = password[lowerBound - 1] == character;
                var upperBoundMatch = password[upperBound - 1] == character;

                if ((lowerBoundMatch && !upperBoundMatch) || (!lowerBoundMatch && upperBoundMatch))
                {
                    validPasswords2++;
                }
            }

            Console.WriteLine($"Part 1 password count: {validPasswords1}");
            Console.WriteLine($"Part 2 password count: {validPasswords2}");
        }
    }
}